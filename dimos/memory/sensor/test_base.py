...
@pytest.fixture
def temp_pickle_dir():
    """Create a temporary directory for PickleDirStore tests."""
...
class TestSensorStore:
    ...
    def test_save_and_load(self, store_factory, store_name, temp_pickle_dir):
        store = store_factory(temp_pickle_dir)
    ...
    def test_find_closest_timestamp(self, store_factory, store_name, temp_pickle_dir):
        store = store_factory(temp_pickle_dir)
        store.save("a", 1.0)
        store.save("b", 2.0)
        store.save("c", 3.0)
    ...
    def test_iter_items(self, store_factory, store_name, temp_pickle_dir):
        store = store_factory(temp_pickle_dir)
        store.save("a", 1.0)
        store.save("c", 3.0)
        store.save("b", 2.0)
    ...
    def test_iter_items_with_range(self, store_factory, store_name, temp_pickle_dir):
        store = store_factory(temp_pickle_dir)
        store.save("a", 1.0)
        store.save("b", 2.0)
        store.save("c", 3.0)
    ...
    def test_empty_store(self, store_factory, store_name, temp_pickle_dir):
        store = store_factory(temp_pickle_dir)
